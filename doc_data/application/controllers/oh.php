...
class oh extends CI_Controller {
	...
	function searchPage()
	{
		...
		if(preg_match('/id\W+aspnetForm.*?(.*?)<\/form>/is',$resultFile,$match))
		{
			...
			$this->errorLog(" -----Search Form Not Found-----!");
		}
	}
	...
	function nextPagebyname($nextResult)
	{
		...
		foreach($nodelist as $n) {
			...
			if(preg_match('/Page\s*(\d*)\s*of\s*(\d*)/is',$page,$match)){
				...
				$this->psslib->appendFile($this->errorLog," totalPage not found on website ");
			}
		}
		if($errorFlag==0)
		{
			$this->psslib->appendFile($this->errorLog," span id 'ctl00_ContentPlaceHolder1_Lbl_PageCnt' not found on website ");
		}
		$second='';
