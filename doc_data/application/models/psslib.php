...
class Psslib extends CI_Model
{
	...
	{
		$getResult=$this->curl->get2($finalimageUrl,'');
	...
	function getPage($url,$content1,$referer='')
	{
		...
		if($content1 == '')
		{
			$resultHtmlPage=$this->curl->get($url,'');
		}
		else
		{
			$resultHtmlPage=$this->curl->post($url,$content1,'');
		}
				...
				if($length1>=2){
					...
					if(preg_match('/(\d+\.\d+\.\d+\.\d+)/is',$proxyInfo[0],$arr)){
						...
						if(($proxyIP!='') && ($proxyPort!='') && ($proxyUser!='') && ($proxyPwd!='')){
							...
							$this->proxyType='HTTP';
							$this->proxyUserPwd=$proxyUser.':'.$proxyPwd;
						...
						$this->appendFile($this->proxyLog,"Proxy : $this->proxyDetail.");
	...
	function setCurlProxy($type='http', $proxyIP=null, $proxyPort=80, $user=null, $pwd=null )
	{
		...
		$this->proxyPort=$proxyPort;
		if(strtolower(trim($type))=='http') { $this->proxyType='HTTP'; }
		elseif(strtolower(trim($type))=='socks4') { $this->proxyType='SOCKS4'; }
		elseif(strtolower(trim($type))=='socks5') { $this->proxyType='SOCKS5'; }
		if(!empty($user) && !empty($pwd)) { $this->proxyUserPwd = "$user:$pwd"; }
	}
	//-----------------------endProxyCode--------------------------------------------
